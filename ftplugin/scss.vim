...
" This removes the comment start (/*) and end (*/) characters.
" It also removes the hashtags (#) used to define ToC indentation.
" Lastly it removes newlines, carriage returns, and trailing whitespace.
" Can be added to or adjusted depending preference
if !exists("g:vct_clean_list")
    let g:vct_clean_list = ['*', '/', '\n', '\r', '\s\+$', '\#']
endif
...
function! s:GetHeadingName(headingLine)
    let l:headingName = a:headingLine
    for substitution in g:vct_clean_list
        let l:headingName = substitute(l:headingName, substitution, '', 'g')
    endfor
...
if g:vct_auto_update_on_save == 1
    autocmd BufWritePre *.{css,scss} if !&diff | exe ':silent! UpdateToc' | endif
endif
